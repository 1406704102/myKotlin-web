    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button size="small" @click="$emit('update:modelValue', false)"
          >取 消</el-button
        >
        <el-button size="small" type="primary" @click="submitForm('user_form')"
          >添加</el-button
        >
      </span>
    </template>
...
<script>
export default {
  props: ["add_dialog_status"],
  emits: { "update:modelValue": null,'revert':null },
  setup(props,context){
      const revert_data=(data)=>{
          context.emit('revert',data)
      }
      return{
          revert_data
      }
  ...
  watch: {
    add_dialog_status() {
      this.dialogVisible = this.add_dialog_status;
